...
export async function sendAudio(audioBlob: Blob) {
  try {
    console.log("Starting to collect device data for audio upload...")
    ...
    console.log("Sending audio with device data to webhook...")

    // Set a timeout for the fetch request
    const controller = new AbortController()
    const timeoutId = setTimeout(() => controller.abort(), 15000) // 15 second timeout for audio uploads
